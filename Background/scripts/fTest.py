...
# Create output file and save model contents CHANGE DIRECTORY EACH TIME
foutDir = "%s/outdir_%s/fTest/output/5to70GeV_0123Merged"%(bwd__,opt.ext)
foutName = "%s/outdir_%s/fTest/output/5to70GeV_0123Merged/CMS-HGG_multipdf_%s.root"%(bwd__,opt.ext,opt.cat)
...
# Plotting
print "\n --> Plotting envelope"
tryMake("/afs/cern.ch/work/a/atsatsos/ULLowmassFGG/CMSSW_10_2_13/src/flashggFinalFit/Background/plots")
tryMake("/afs/cern.ch/work/a/atsatsos/ULLowmassFGG/CMSSW_10_2_13/src/flashggFinalFit/Background/plots/%s"%opt.year)
plotPdfMap(model,model.envelopePdfs,opt.plotBlindingRegion,_outdir="/afs/cern.ch/work/a/atsatsos/ULLowmassFGG/CMSSW_10_2_13/src/flashggFinalFit/Background/plots/%s"%opt.year,_cat=opt.cat)
