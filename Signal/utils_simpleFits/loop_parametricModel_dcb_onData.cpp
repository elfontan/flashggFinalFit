...
void loop_parametricModel_dcb_onData(TString year="2018", TString cat="cat1"){
  ...
  for (size_t massindex = 0; massindex < v_mass.size(); ++massindex) {  
   ...
   //mean1.setVal(v_dm0.at(massindex));
    ...
    if (cat=="cat0"){
      c_sig.SaveAs(Form("/eos/user/e/elfontan/www/LowMassDiPhoton/diphotonBDT/ParametricBDT/TensorFlow/SignalModeling/newNtuples_newTraining_NN0p907/TESTS/mNom40_tuningParams/sig_ggH_cat0_"+year+"_dcb_mass%d.png", int(v_mass.at(massindex))));
    }else if (cat=="cat1"){
      c_sig.SaveAs(Form("/eos/user/e/elfontan/www/LowMassDiPhoton/diphotonBDT/ParametricBDT/TensorFlow/SignalModeling/newNtuples_newTraining_NN0p907/TESTS/mNom40_tuningParams/sig_ggH_cat1_"+year+"_dcb_mass%d.png", int(v_mass.at(massindex))));
    }else {
      // Handle the case when neither category is selected
      cout << "Please select a category to process." << endl;
    }
